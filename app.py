...
def main() -> None:
    ...
    building_location = parse_location_text(building_location_text)
    ...
    with left_col:
        ...
        if str(community_row.get('district','')).strip(): chip_html.append(f"<span class='chip'>{community_row.get('district')}</span>")
        if str(community_row.get('far_ratio','')).strip(): chip_html.append(f"<span class='chip'>容积率 {community_row.get('far_ratio')}</span>")
        if str(community_row.get('build_year','')).strip(): chip_html.append(f"<span class='chip'>楼龄 {community_row.get('build_year')}</span>")
        ...
        result = score_engine.final_score(DEFAULT_BASE_SCORE, zone_adjust, int(noise_summary.get('total_penalty',0)), community_row.get('far_ratio',''), community_row.get('build_year',''))
        st.markdown(f"**位置解释**：{zone_row.get('description','')}" )
        st.markdown(f"<div class='tiny-note'>公式：75 + 分区修正 {result['zone_adjust']:+d} + 建筑加分 {result['build_bonus']:+d} - 密度惩罚 {result['density_penalty']:+d} - 周边噪音点惩罚 {result['noise_penalty']:+d} = {result['final_score']}</div>", unsafe_allow_html=True)
    with right_col:
    ...
    if signals:
        for sig in signals:
            st.markdown(f"<div class='signal-row'><div><strong>{sig.get('label','')}</strong><br><span class='tiny-note'>{sig.get('detail','')}</span></div><div><strong>{sig.get('distance_m','-')}m</strong>｜惩罚 {int(sig.get('penalty',0)):+d}</div></div>", unsafe_allow_html=True)
