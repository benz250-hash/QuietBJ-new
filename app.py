...
def render_styles() -> None:
    ...
    st.markdown(
        ...
        .block-container {{max-width: 1180px !important; padding-top: 0 !important; padding-left: 1.2rem !important; padding-right: 1.2rem !important; padding-bottom: 4rem !important;}}
        .bg-layer {{position: fixed; inset: 0; z-index: -20; pointer-events:none; background-image: linear-gradient(180deg, rgba(9,17,14,0.24), rgba(9,17,14,0.58)), url("data:image/jpeg;base64,{bg_base64}"); background-size: cover; background-position: center center; background-repeat: no-repeat;}}
        .topbar {{display:flex; justify-content:space-between; align-items:center; padding:16px 0 0; color:#fff;}}
        .brand {{font-size:15px; font-weight:600; letter-spacing:0.10em; opacity:0.42;}}
        .hero {{min-height: 58vh; display:flex; align-items:flex-start; justify-content:center; padding-top: 6vh;}}
        .hero-box {{width:min(920px,96%); text-align:center; color:#fff; margin:0 auto;}}
        .hero-kicker {{font-size:11px; letter-spacing:0.24em; text-transform:uppercase; opacity:0.84; margin-bottom:12px;}}
        .hero-title {{font-size: clamp(44px, 6vw, 82px); font-weight:800; line-height:1.04; margin:0; text-shadow:0 8px 30px rgba(0,0,0,0.22);}}
        .hero-sub {{font-size:16px; line-height:1.75; max-width:760px; margin:14px auto 0; color:rgba(255,255,255,0.96);}}
        .hero-note {{display:inline-block; margin-top:16px; padding:10px 16px; border-radius:999px; background:rgba(255,255,255,0.08); border:1px solid rgba(255,255,255,0.18); color:rgba(255,255,255,0.92); font-size:11px; letter-spacing:0.04em;}}
        .search-shell {{max-width: 900px; margin: 18px auto 0; padding: 14px; border-radius: 22px; background: rgba(255,255,255,0.08); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.14); box-shadow: 0 20px 60px rgba(0,0,0,0.14);}}
        .search-foot {{margin-top: 8px; text-align:center; color: rgba(255,255,255,0.72); font-size: 12px;}}
        div[data-testid="stTextInputRootElement"] input {{height: 54px !important; border-radius: 16px !important; border: none !important; box-shadow: none !important; background: rgba(255,255,255,0.97) !important; font-size: 17px !important;}}
        div[data-testid="stTextInputRootElement"] input::placeholder {{color: rgba(65,74,70,0.42) !important;}}
        div[data-testid="stWidgetLabel"], div[data-testid="InputInstructions"] {{display: none !important;}}
        div.stButton > button, div[data-testid="stFormSubmitButton"] > button {{height: 48px; border-radius: 14px; font-weight: 700; box-shadow:none !important;}}
        div[data-testid="stFormSubmitButton"] > button[kind="primary"], div.stButton > button[kind="primary"] {{background:#173a2d !important; border:1px solid #173a2d !important; color:white !important;}}
        div[data-testid="stFormSubmitButton"] > button[kind="secondary"], div.stButton > button[kind="secondary"] {{background: rgba(255,255,255,0.88) !important; border:1px solid rgba(24,37,31,0.08) !important; color:#31443b !important;}}
        .results-stage {{margin-top: 32px; padding: 22px; border-radius: 28px; background: rgba(246,248,246,0.94); box-shadow: 0 22px 70px rgba(12,18,15,0.14); border:1px solid rgba(19,34,26,0.06);}}
        .stage-head {{font-size: 13px; letter-spacing:0.18em; text-transform:uppercase; color:#738078; margin-bottom: 14px;}}
        .result-card {{background: white; border:1px solid rgba(22,36,29,0.06); border-radius: 22px; padding: 22px; box-shadow: 0 10px 34px rgba(23,34,28,0.05);}}
        .score-card {{background: linear-gradient(180deg, #163a2c 0%, #1b583d 100%); color:white; border-radius: 24px; padding: 26px; box-shadow: 0 18px 42px rgba(21,58,43,0.24);}}
        .score-kicker {{opacity:0.78; letter-spacing:0.16em; text-transform:uppercase; font-size:12px;}}
        .score-number {{font-size:88px; line-height:1; font-weight:800; margin: 10px 0 6px;}}
        .eyebrow {{font-size: 12px; text-transform: uppercase; letter-spacing: 0.14em; color:#7a877f;}}
        .headline {{font-size: 40px; line-height:1.1; font-weight: 800; color:#14231d; margin: 6px 0 8px;}}
        .summary-line {{font-size: 16px; color:#2c3d35; line-height:1.7; margin: 4px 0 0;}}
        ...
        .deduct-left {{display:flex; flex-direction:column; gap:4px;}}
        .deduct-title {{font-size:16px; font-weight:700; color:#1b2a23;}}
        .deduct-detail {{font-size:13px; color:#728078;}}
        .deduct-right {{font-size:16px; font-weight:700; color:#173a2d; white-space:nowrap;}}
        .section-title {{font-size: 26px; font-weight: 800; color:#17251f; margin:0 0 6px;}}
        .section-sub {{font-size: 14px; color:#6f7b73; line-height:1.7; margin-bottom: 10px;}}
        .anchor-gap {{height: 6px;}}
        div[data-testid="stSelectbox"] > div {{border-radius: 14px !important;}}
        .debug-box {{background:#f7faf7; border:1px solid #e3ebe6; border-radius:18px; padding:14px;}}
        @media (max-width: 900px) {{
            .block-container {{padding-left: 0.9rem !important; padding-right: 0.9rem !important;}}
            .hero {{min-height: 50vh; padding-top: 5vh;}}
...
def render_search() -> tuple[str, bool, bool]:
    ...
    with center:
        ...
        with st.form('hero_search', clear_on_submit=False):
            query = st.text_input('hero_query', placeholder='输入北京小区或楼栋地址，例如：新龙城6号楼 / 花家地西里2号楼', label_visibility='collapsed')
            c1, c2, c3 = st.columns([5.0, 1.3, 1.0])
            with c2:
        ...
        st.markdown('<div class="search-foot">建议输入：小区名 + 楼号。系统会先识别小区，再围绕更接近楼栋的坐标测算外部噪音暴露。</div>', unsafe_allow_html=True)
...
def parse_geocode_result(query: str, community_repo: CommunityRepository, amap: AMapProvider) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any] | None, str]:
    ...
    return community_row, tips, regeo, building_location_text
...
def render_overview_card(query: str, community_row: dict[str, Any], result: dict[str, int], signals: list[dict[str, Any]]) -> None:
    ...
    with left:
        st.markdown('<div class="eyebrow">Result Overview</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="headline">{community_row.get("community_name", "目标小区")}</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="summary-line">{build_summary_line(signals)}</div>', unsafe_allow_html=True)
        ...
        st.markdown(''.join(pills) + '</div>', unsafe_allow_html=True)
        st.markdown(
        ...
        st.markdown(''.join([f'<div class="metric-box"><div class="metric-label">{a}</div><div class="metric-value">{b}</div><div class="metric-note">{c}</div></div>' for a,b,c in metric_html]) + '</div>', unsafe_allow_html=True)
    with right:
        ...
        st.write('系统按楼栋位置、道路距离、商业暴露、学校医院和轨道交通进行估算，用于快速判断这套房是否值得继续看。')
        st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
def render_penalty_card(noise_summary: dict[str, Any]) -> None:
    ...
    st.markdown('<div class="result-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">扣分来源</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-sub">先看拖分项，再决定要不要继续实勘。这一层只展示真正影响当前楼栋体感的主要外部因素。</div>', unsafe_allow_html=True)
    if not signals:
        st.info('当前没有识别到显著的外部噪音暴露，系统没有形成有效扣分。')
    else:
        for sig in sorted(signals, key=lambda x: int(x.get('penalty', 0)), reverse=True):
            st.markdown(
                f'<div class="deduct-row"><div class="deduct-left"><div class="deduct-title">{sig.get("label", "")}</div><div class="deduct-detail">{sig.get("detail", "")}</div></div><div class="deduct-right">{sig.get("distance_m", "-")}m ｜ -{int(sig.get("penalty", 0))}</div></div>',
                unsafe_allow_html=True,
            )
...
def render_position_card(zone_options: list[dict[str, Any]], community_row: dict[str, Any], score_engine: ScoreEngine, noise_penalty: int) -> dict[str, int]:
    ...
    st.markdown('<div class="result-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">楼栋位置修正</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-sub">这里不是让用户“调参数”，而是把楼栋放回真实位置语境里：临街、中央区、内排安静区，体感差异会很明显。</div>', unsafe_allow_html=True)
    ...
    for col, title, value, note in [
        ...
        (c2, '楼栋位置', f'{result["zone_adjust"]:+d}', selected_name),
        (c3, '建筑加分', f'{result["build_bonus"]:+d}', '楼龄代理值'),
        (c4, '密度调整', f'-{result["density_penalty"]}', '容积率代理值'),
    ]:
        with col:
            st.markdown(f'<div class="metric-box"><div class="metric-label">{title}</div><div class="metric-value">{value}</div><div class="metric-note">{note}</div></div>', unsafe_allow_html=True)
    st.markdown(f"<div class='subtle' style='margin-top:12px;'>位置解释：{zone_row.get('description', '按当前楼栋位置修正')}。</div>", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
    return result
...
def main() -> None:
    ...
    community_row, tip_list, regeo, building_location_text = parse_geocode_result(query, community_repo, amap)
    geocode_full = amap.geocode(query) if amap.enabled() else None
    geocode_clean = amap.geocode(strip_unit_details(query)) if amap.enabled() and not geocode_full else None
    geocode_used = geocode_full or geocode_clean
    ...
    st.markdown('<div class="results-stage">', unsafe_allow_html=True)
    st.markdown('<div class="stage-head">Residential Result</div>', unsafe_allow_html=True)

    # Position first to compute final score, but place overview first visually after computing.
    result = render_position_card(zone_options, community_row, score_engine, int(noise_summary.get('total_penalty', 0)))
    st.markdown('<div class="anchor-gap"></div>', unsafe_allow_html=True)
